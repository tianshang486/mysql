...
2.  在页面左上角，选择实例所在地域。
    ...
    ![地域截图](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/7882/155174942937169_zh-CN.png)
...
5.  在日志管理页面选择查询**错误日志**、**慢日志明细**、**慢日志统计**或者**主备切换日志**，选择时间范围，单击**查询**。
    ...
    |慢日志明细|记录1个月内数据库中执行时间超过1s（MySQL可以通过修改参数设置中的long\_query\_time参数来修改这个时间阈值）的SQL语句，并进行相似语句去重。|
    |慢日志统计|对1个月内数据库中执行时间超过1秒（MySQL可以通过修改参数设置中的long\_query\_time参数来修改这个时间阈值）的SQL语句进行统计汇总，给出慢查询日志的分析报告。|
