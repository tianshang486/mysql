...
-   数据同步的目标RDS实例已存在，RDS实例创建方法请参见[创建RDS实例](https://www.alibabacloud.com/help/zh/doc-detail/26117.htm)。
...
-   一对一双向同步
    ...
    **说明：** 如需实现双向同步，请参见[RDS MySQL实例间的双向同步](/intl.zh-CN/数据同步/MySQL间数据同步/RDS MySQL实例间的双向同步.md)。
...
在正式配置数据同步作业之前，您需要[为自建MySQL创建账号并设置binlog](/intl.zh-CN/准备工作/为自建MySQL创建账号并设置binlog.md)。
