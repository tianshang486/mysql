...
-   存储类型：分为SSD本地盘与SSD云盘。具体请参见[存储类型](cn.zh-CN/云数据库RDS简介/存储类型.md)。
...
-   [MongoDB](../../../../../cn.zh-CN/产品简介/什么是云数据库MongoDB.md)：提供稳定可靠、弹性伸缩、完全兼容MongoDB协议的数据库服务。数据结构多样时，可以选择将结构化数据存储在RDS，将非结构化数据存储在MongoDB，满足业务的多样化存储需求。
