...
-   [创建RDS实例（新版）](#section_raf_exe_9nw)
 ...
 **说明：** 不同地域支持的数据库类型不同，请以实际界面为准。
 ...
 |
    |版本|指PPAS的版本。RDS for PPAS支持的版本包括PPAS 9.3、10。 **说明：** 不同地域所支持的版本不同，请以实际界面为准。
...
## 创建RDS实例（新版） {#section_raf_exe_9nw .section}
...
3.  设置以下参数。
 ...
 |
    |**数据库类型**|无|即数据库引擎的类型，这里选择**PPAS数据库**。 **说明：** 不同地域支持的数据库类型不同，请以实际界面为准。

 |
    |**版本系列**|**版本**|指PPAS的版本。当前支持PPAS 9.3、10。|
    |**系列**|**高可用版**：一个主节点和一个备节点，经典高可用架构。 **说明：** 不同数据库版本支持的系列不同，请以实际界面为准。关于各个系列的详细介绍，请参见[产品系列概述](../cn.zh-CN/云数据库RDS简介/产品系列/产品系列概述.md)。

 |
    |**部署形态**|**存储类型**|**本地SSD盘**：与数据库引擎位于同一节点的SSD盘。将数据存储于本地SSD盘，可以降低I/O延时。 更多信息，请参见[存储类型](../cn.zh-CN/云数据库RDS简介/存储类型.md)。
