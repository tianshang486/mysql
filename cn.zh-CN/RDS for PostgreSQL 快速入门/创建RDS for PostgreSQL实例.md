...
您可以通过阿里云RDS管理控制台或API创建RDS实例。本文介绍如何通过控制台创建RDS for PostgreSQL实例。
...
-   [创建PostgreSQL 10高可用版（本地盘）/PostgreSQL 10基础版/PostgreSQL 9.4（新版）](#section_6no_pdj_746)
...
1.  进入[新版PostgreSQL实例创建](https://common-buy.aliyun.com/?productCode=postgresql&chargeType=prepay&#/buy)页面。
2.  选择**包年包月**或**按量付费**。
...
## 创建PostgreSQL 10高可用版（本地盘）/PostgreSQL 10基础版/PostgreSQL 9.4（新版） {#section_6no_pdj_746 .section}

1.  进入[RDS实例创建](https://rds-buy.aliyun.com/nextBuy/?#/create/rds/mysql)页面。

    **说明：** 您也可以在当前创建RDS实例页面上方单击**返回旧版**切换到旧版创建RDS实例页面。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/7814/156885791460573_zh-CN.png)
...
2.  设置以下参数。
 ...
 |
    ...
    -   请确保RDS实例与需要连接的ECS实例创建于同一个地域，否则它们无法通过内网互通，只能通过外网互通，无法发挥最佳性能。
 |
    |**类型**|数据库引擎的类型和版本，这里选择**PostgreSQL**。当前支持PostgreSQL 9.4、10 **说明：** 不同地域支持的数据库类型不同，请以实际界面为准。
...
5.  单击**下一步：确认订单**，确认**参数配置**，选择**购买量**和**购买时长**（仅包年包月实例），勾选服务协议，单击**去支付**完成支付。
    ...
    **说明：** 如果有优惠券，可以在本页面选择对应的优惠券。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/7814/156885791452773_zh-CN.png)
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/7846/156885791449667_zh-CN.png)
