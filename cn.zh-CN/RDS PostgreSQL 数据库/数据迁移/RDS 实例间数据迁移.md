...
5.  配置迁移任务的源库及目标库信息。
    ...
    ![RDS实例间迁移](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/2767559951/p49618.png)
    ...
    |数据库账号|填入源RDS实例的数据库账号，权限要求请参见[数据库账号的权限要求](#d7e80)。|
...
当RDS实例的引擎为**MySQL**时，该参数才会出现并需要配置。 |
    ...
    |数据库账号|填入目标RDS实例的数据库账号，权限要求请参见[数据库账号的权限要求](#d7e80)。|
